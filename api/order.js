...
export default async function handler(req, res) {
  ...
  try {
    ...
    const itemsText = orderItems
      .map(item =>
        `• ${escapeHtml(item.name)} × ${item.qty} — ${item.sum.toLocaleString("uz-UZ")} so‘m`
      )
      .join("\n");
...
${typeText}
${customer.address ? `📍 <b>Manzil:</b> ${escapeHtml(customer.address)}` : ""}
...
`;
    ...
    /*
      Telegram Mini App orqali kelgan mijozning ID'si.
      Aynan shu ID keyinchalik mijozga
      "Buyurtmangiz qabul qilindi" yuborish uchun kerak.
    */
    const customerId = user?.id ? String(user.id) : "";

    /*
      callback_data maksimal 64 byte bo‘lishi mumkin.
      Shuning uchun ID va order ID'ni qisqa formatda yuboramiz.
    */
    const keyboard = {
      inline_keyboard: [
        [
          {
            text: "✅ Qabul qilish",
            callback_data: `accept_${orderId}_${customerId}`
          },
          {
            text: "❌ Rad etish",
            callback_data: `reject_${orderId}_${customerId}`
          }
        ]
      ]
    };
